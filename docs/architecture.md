...
v1.5.7 keeps the browser stack baked at build time:
...
Release inputs that do not have a package-manager lock are checked during the Docker build. Claude Code and Junie use exact supported versions. Cursor is bound to architecture-specific build archives and verified launcher and bundled Node input hashes, then its bundled Node is removed and linked to HolyClaude's patched Node 26.7.0 runtime. s6-overlay and fzf are checked against upstream release checksums. Azure CLI, GitHub CLI, Docker CE CLI, and Docker Compose plugin use pinned bootstrap inputs and installed package assertions. The Docker packages are client-only; Docker host access needs an explicit runtime socket or remote-daemon configuration. The full image rebuilds the Bookworm FFmpeg package set and Azure CLI's cryptography wheel with exact upstream security patches, while Netlify's image-size copy receives the matching compiled parser fixes. The release inventory in `security/immutable-inputs.yml` binds those values to v1.5.7 and expires the review instead of letting it silently age.
