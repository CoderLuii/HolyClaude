...
> **Зачем эти права браузера?** В этой версии сохраняется текущий браузерный профиль HolyClaude. `SYS_ADMIN` и `seccomp=unconfined` расширяют права процессов и снижают изоляцию; `SYS_PTRACE` нужен для отладки. Оставьте этот профиль без изменений для v1.5.2 и рассматривайте hardening как отдельное изменение.
