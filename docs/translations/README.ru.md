...
| **Обновления** | `docker compose pull && docker compose up -d` | Обновлять 50 инструментов вручную, молясь, что ничего не сломается |
...
>
> Размер загрузки зависит от выпуска и архитектуры. Сжатый размер каждого тега можно проверить на [Docker Hub](https://hub.docker.com/r/coderluii/holyclaude/tags). Docker, Synology Container Manager и файловые системы NAS могут показывать больший размер после распаковки слоев.
