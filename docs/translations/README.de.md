...
> HolyClaude betreibt keinen Credential-Relay. Gebundelte Tools lesen Anmeldedaten aus Containerdateien, Bind-Mounts oder Umgebungsvariablen und kontaktieren konfigurierte Anbieter direkt.
...
> **HolyClaude ist kostenlos und Open Source.** Du zahlst deinen KI-Anbietern nur für die Nutzung. HolyClaude betreibt keinen Credential-Relay. Gebundelte Tools lesen Anmeldedaten aus Containerdateien, Bind-Mounts oder Umgebungsvariablen und kontaktieren konfigurierte Anbieter direkt.
...
>
> Die Downloadgröße hängt von Release und Architektur ab. Die komprimierte Größe jedes Tags findest du auf [Docker Hub](https://hub.docker.com/r/coderluii/holyclaude/tags). Docker, Synology Container Manager und NAS-Dateisysteme können nach dem Entpacken eine größere Größe anzeigen.
...
> **Warum diese Browser-Rechte?** Diese Version behält HolyClaudes aktuelles Browser-Profil bei. `SYS_ADMIN` und `seccomp=unconfined` erweitern Prozessrechte und verringern die Isolation; `SYS_PTRACE` dient dem Debugging. Lass dieses Profil für v1.5.5 unverändert und behandle Hardening als separaten Schritt.
...
| `PUID` | `1000` | Container-Benutzer-ID fuer Docker-Remapping; rootless Podman nutzt `docker-compose.podman-rootless.yaml` |
| `PGID` | `1000` | Container-Gruppen-ID fuer Docker-Remapping; rootless Podman nutzt `docker-compose.podman-rootless.yaml` |
| `NODE_OPTIONS` | `docker-compose.full.yaml: --max-old-space-size=4096` | Node.js-Heap-Speichergrenze in MB |
| `GIT_USER_NAME` | `HolyClaude User` | Git-Commit-Autor (nur gesetzt, wenn kein Wert vorhanden ist) |
| `GIT_USER_EMAIL` | `noreply@holyclaude.local` | Git-Commit-E-Mail (nur gesetzt, wenn kein Wert vorhanden ist) |
...
| `HOLYCLAUDE_DESLOPPIFY_SETUP` | `off` | Optionale globale Desloppify-Skill-Einrichtung. Gueltig: `off`, `all`, `claude`, `codex`, `gemini`, `opencode` oder kommagetrennte Auswahl |
...
| `desloppify`, `bandit`, `tree-sitter` | Code-Qualitaetsscans, Python-Sicherheitschecks, parsergestuetzte Codeanalyse |
...
**Nur in dieser Variante:** Junie (`junie`), OpenCode (`opencode`) und Pi (`pi`).
...
| Paket | Wofür |
|---------|---------------|
| `reportlab`, `weasyprint`, `cairosvg`, `fpdf2`, `PyMuPDF`, `img2pdf` | Bibliotheken zum Erstellen, Lesen, Konvertieren und Zusammenführen von PDFs. |
...
| **Junie** | `junie` | JetBrains AI-Abonnement | JetBrains-Konto erforderlich; **Nur im Full-Image** |
| **OpenCode** | `opencode` | Konfiguration über TUI | Unterstützt OpenRouter und mehrere Anbieter; nur Full-Image |
| **Pi Coding Agent** | `pi` | Über Pi konfigurieren | Unterstützt mehrere Anbieter; **Nur im Full-Image** |
...
Desloppify ist in beiden Images als `desloppify` enthalten. Standardmaessig bleibt es passiv: HolyClaude startet keine Scans, erstellt kein `.desloppify/`, bearbeitet keine `.gitignore` und aendert keine gemounteten Workspaces, solange du Desloppify nicht selbst ausfuehrst.
...
Nach einem Scan fuege `.desloppify/` zur `.gitignore` dieses Projekts hinzu.

`HOLYCLAUDE_DESLOPPIFY_SETUP=off` schaltet nur die automatische globale Skill-Einrichtung aus; die CLI bleibt installiert. `all` bedeutet `claude,codex,gemini`. `opencode` ist nur im Full-Image verfuegbar und wird nicht in `all` aufgenommen. Kombiniere `claude` und `opencode` nicht automatisch, weil OpenCode Claude-kompatible Skills aus `~/.claude/skills` erkennen kann.

Manuelle Upstream-Ziele: `cursor`, `copilot`, `windsurf`, `qwen`, `amp`, `rovodev`, `droid`, `hermes`.
...
Siehe die vollständige Einrichtungsanleitung: **[docs/ollama.md](../ollama.md)**
...
2. **Nur beim ersten Start** — `bootstrap.sh` läuft einmalig. Kopiert Standardeinstellungen und Memory-Template. Erstellt eine Sentinel-Datei (`.holyclaude-bootstrapped`), sodass es nie wieder läuft. Deine Anpassungen sind ab diesem Punkt sicher.
...
Siehe [docs/architecture.md](../architecture.md) für den vollständigen technischen Tiefen-Einblick — einschließlich warum wir s6 statt supervisord gewählt haben, warum Plugins ins Image eingebaut sind und warum `runuser` statt `su`.
...
|------|-------------------|-------------|-------------------|
| Claude-Einstellungen und gespeicherte Tool-Konfiguration | `/home/claude/.claude` | `./data/claude` | **Ja** |
| Claude Code Session (OAuth, Onboarding) | `/home/claude/.claude.json` | `./data/claude/.claude.json.persist` | **Ja** |
| Globale Git- und XDG-Konfiguration | `/home/claude/.gitconfig`, `/home/claude/.config/git` | `./data/claude/.gitconfig`, `./data/claude/.config/git` | **Ja** |
| GitHub-CLI-Konfiguration und -Authentifizierung | `/home/claude/.config/gh` | `./data/claude/.config/gh` | **Ja** |
...
- Dateibasierte Anthropic-Authentifizierung und API-Key-Einstellungen von Claude Code
- Claude Code-Einstellungen, Memory (`CLAUDE.md`) und OAuth-Session (kein erneutes Login)
- Dein gesamter Code in `./workspace`
- Globale Git-Konfiguration, Aliase und XDG-Git-Einstellungen
- GitHub-CLI-Konfiguration und -Authentifizierung
- Dateibasierte Codex-, Gemini- und Cursor-Konfiguration und -Authentifizierung unter `/home/claude/.claude`. Über Umgebungsvariablen bereitgestellte Schlüssel bleiben in Compose oder der Host-Umgebung.
...
> **`./data/claude/`:** Lösche den Ordner nicht vollständig. Er enthält gespeicherte Claude-Code-Daten und kann ab v1.5.5 GitHub-CLI-Zugangsdaten enthalten. Committe ihn nicht, teile ihn nicht breit und speichere ihn nicht in unverschlüsselten Backups.
...
Siehe [Konfigurationsdokumentation](../configuration.md#notifications-apprise) für alle unterstützten Variablen und URL-Formate.
...
> Aktualisierst du von einer Version vor v1.5.5? Migriere vorhandene globale Git- oder `gh auth`-Daten aus dem laufenden Container, bevor du ihn neu erstellst. Folge dazu der [Git- und GitHub-CLI-Wiederherstellung](../troubleshooting.md#git-identity-or-gh-auth-disappears-after-recreate). Wenn du den alten Container zuerst entfernst, können diese Daten verloren gehen.
...
Führe `cloudcli update` oder `npm install -g @cloudcli-ai/cloudcli@latest` nicht im Container aus. HolyClaude enthält eine gepatchte CloudCLI-Laufzeit; aktualisiere mit `docker compose pull && docker compose up -d`.
...
Siehe [docs/troubleshooting.md](../troubleshooting.md) für die vollständige Anleitung einschließlich aller SMB/CIFS-Eigenheiten und der vollständigen Geschichte der gefundenen und behobenen Bugs.
