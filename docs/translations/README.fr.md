...
> HolyClaude n'opère aucun relais d'identifiants. Les outils inclus lisent les identifiants depuis des fichiers du conteneur, des bind mounts ou des variables d'environnement, puis contactent directement les fournisseurs configurés.
...
| **Persistance** | Les bind mounts conservent la configuration des outils basée sur des fichiers et le workspace après une reconstruction | Comprendre les volumes Docker, déboguer "pourquoi c'est un répertoire et pas un fichier" |
| **Mises à jour** | `docker compose pull && docker compose up -d` | Mettre à jour 50 outils manuellement, prier pour que rien ne casse |
...
> **HolyClaude est gratuit et open source.** Vous ne payez vos fournisseurs IA que pour l'usage. HolyClaude n'opère aucun relais d'identifiants. Les outils inclus lisent les identifiants depuis des fichiers du conteneur, des bind mounts ou des variables d'environnement, puis contactent directement les fournisseurs configurés.
...
>
> La taille du téléchargement varie selon la version et l'architecture. Consultez la taille compressée de chaque tag sur [Docker Hub](https://hub.docker.com/r/coderluii/holyclaude/tags). Docker, Synology Container Manager et les systèmes de fichiers NAS peuvent afficher une taille plus grande après extraction des couches.
...
| `PUID` | `1000` | ID utilisateur du conteneur pour le remappage Docker; Podman rootless utilise `docker-compose.podman-rootless.yaml` |
| `PGID` | `1000` | ID de groupe du conteneur pour le remappage Docker; Podman rootless utilise `docker-compose.podman-rootless.yaml` |
| `NODE_OPTIONS` | `docker-compose.full.yaml: --max-old-space-size=4096` | Limite mémoire heap Node.js en Mo |
| `GIT_USER_NAME` | `HolyClaude User` | Auteur des commits Git (défini uniquement si la valeur manque) |
| `GIT_USER_EMAIL` | `noreply@holyclaude.local` | Email des commits Git (défini uniquement si la valeur manque) |
...
| `HOLYCLAUDE_DESLOPPIFY_SETUP` | `off` | Configuration globale optionnelle des skills Desloppify. Valeurs: `off`, `all`, `claude`, `codex`, `gemini`, `opencode` ou sous-ensembles separes par des virgules |
...
<details>
<summary><strong>Node.js 26 + paquets npm globaux</strong></summary>
...
| `desloppify`, `bandit`, `tree-sitter` | Scans de qualite du code, controles de securite Python, analyse de code avec parser |
...
| `openssh-client`, `openssh-server`, `mosh` | SSH sortant et accès SSH/Mosh optionnel au conteneur |
...
**Uniquement dans cette variante :** Junie (`junie`), OpenCode (`opencode`) et Pi (`pi`).
...
| Paquet | À quoi ça sert |
|---------|---------------|
| `reportlab`, `weasyprint`, `cairosvg`, `fpdf2`, `PyMuPDF`, `img2pdf` | Bibliothèques pour générer, lire, convertir et fusionner des PDF. |
...
| **Junie** | `junie` | Abonnement JetBrains AI | Compte JetBrains requis; **Image complète uniquement** |
| **OpenCode** | `opencode` | Configurer via TUI | Supporte OpenRouter et plusieurs fournisseurs; image full seulement |
| **Pi Coding Agent** | `pi` | Configurer via Pi | Supporte plusieurs fournisseurs; **Image complète uniquement** |
...
Desloppify est fourni dans les deux images via la commande `desloppify`. Par defaut, il reste passif: HolyClaude ne lance pas de scan, ne cree pas `.desloppify/`, ne modifie pas `.gitignore` et ne touche pas aux workspaces montes sauf si vous lancez Desloppify vous-meme.
...
Apres un scan, ajoutez `.desloppify/` au `.gitignore` du projet concerne.

`HOLYCLAUDE_DESLOPPIFY_SETUP=off` desactive seulement la configuration globale automatique des skills; la CLI reste installee. `all` signifie `claude,codex,gemini`. `opencode` est reserve a l'image full et n'est pas inclus dans `all`. Ne combinez pas `claude` et `opencode` en configuration automatique, car OpenCode peut deja decouvrir les skills compatibles Claude depuis `~/.claude/skills`.

Cibles upstream manuelles: `cursor`, `copilot`, `windsurf`, `qwen`, `amp`, `rovodev`, `droid`, `hermes`.
...
Consultez le guide de configuration complet : **[docs/ollama.md](../ollama.md)**
...
1. **Le conteneur démarre** — `entrypoint.sh` s'exécute en tant que root. Remapping de l'UID/GID pour correspondre à votre utilisateur hôte, restauration de la session Claude Code enregistrée avant que bootstrap puisse la toucher, vérification s'il s'agit d'un premier démarrage.

2. **Premier démarrage uniquement** — `bootstrap.sh` s'exécute une seule fois. Copie les paramètres par défaut et le modèle de mémoire. Crée un fichier sentinelle (`.holyclaude-bootstrapped`) pour qu'il ne s'exécute plus jamais. Vos personnalisations sont protégées à partir de ce moment.

3. **s6-overlay prend le contrôle en tant que PID 1** — Ce n'est pas supervisord. C'est [s6-overlay](https://github.com/just-containers/s6-overlay), conçu spécifiquement pour Docker. Supervise CloudCLI, Xvfb et la synchronisation de session Claude. Redémarre automatiquement en cas de crash. Transmet les signaux. Récolte les zombies. S'arrête proprement.
...
Consultez [docs/architecture.md](../architecture.md) pour la plongée technique complète — notamment pourquoi nous avons choisi s6 plutôt que supervisord, pourquoi les plugins sont intégrés dans l'image, et pourquoi `runuser` plutôt que `su`.
...
|------|-------------------|-------------|-------------------|
| Paramètres Claude et configuration persistante des outils | `/home/claude/.claude` | `./data/claude` | **Oui** |
| Session Claude Code (OAuth, onboarding) | `/home/claude/.claude.json` | `./data/claude/.claude.json.persist` | **Oui** |
| Configuration globale Git et XDG | `/home/claude/.gitconfig`, `/home/claude/.config/git` | `./data/claude/.gitconfig`, `./data/claude/.config/git` | **Oui** |
| Configuration et authentification GitHub CLI | `/home/claude/.config/gh` | `./data/claude/.config/gh` | **Oui** |
...
HolyClaude restaure la session Claude Code enregistrée avant que le démarrage puisse créer un nouveau fichier par défaut. Les rebuilds et recreates ne remplacent donc pas une vraie session OAuth/API par un état d'onboarding.

### Ce qui survit à `docker compose down && docker compose up` :
- Authentification Anthropic basée sur des fichiers et paramètres de clé API de Claude Code
- Paramètres Claude Code, mémoire (`CLAUDE.md`) et session OAuth (sans reconnexion)
- Tout votre code dans `./workspace`
- Configuration globale, alias et réglages XDG de Git
- Configuration et authentification GitHub CLI
- Configuration et authentification de Codex, Gemini et Cursor basées sur des fichiers sous `/home/claude/.claude`. Les clés fournies par des variables d'environnement restent dans Compose ou l'environnement hôte.
...
> **`./data/claude/`:** Ne supprimez pas entièrement ce dossier. Il contient des données Claude Code et peut contenir des identifiants GitHub CLI depuis v1.5.5. Ne le commitez pas, ne le partagez pas largement et ne le stockez pas dans une sauvegarde non chiffrée.
...
HolyClaude prépare ce répertoire avant le démarrage de CloudCLI. Les nouveaux volumes Docker héritent du propriétaire `claude:claude` ; les volumes locaux existants sont corrigés selon `PUID`/`PGID` lors d'un démarrage Docker normal en root. Si le montage est en lecture seule ou ne peut pas être corrigé, le démarrage s'arrête avec le chemin et l'UID/GID au lieu de répéter `unable to open database file`. Pour Podman rootless, utilisez le fichier Compose keep-id fourni avec `:Z` ; `:U` réécrit les propriétaires sur l'hôte et n'est pas la valeur par défaut.
...
Claude Code s'exécute en mode **`acceptEdits`** par défaut. C'est le paramètre livré dans l'image :
...
> **Le mode bypass signifie que Claude exécute des commandes sans confirmation.** Il est puissant, mais il peut aussi exécuter rapidement des commandes destructrices. Gardez le paramètre livré `acceptEdits` sauf si vous faites confiance au workspace et à chaque prompt.
...
3. **Gardez le paramètre livré `acceptEdits`**, sauf si vous avez une raison claire d'utiliser `bypassPermissions`
...
Consultez la [documentation de configuration](../configuration.md#notifications-apprise) pour toutes les variables supportées et les formats d'URL.
...
> Vous mettez à jour une version antérieure à v1.5.5 ? Migrez la configuration Git globale ou l'état `gh auth` du conteneur actuel avant de le recréer. Suivez la procédure de [récupération Git et GitHub CLI](../troubleshooting.md#git-identity-or-gh-auth-disappears-after-recreate). Supprimer d'abord l'ancien conteneur peut faire perdre ces données.
...
N'exécutez pas `cloudcli update` ni `npm install -g @cloudcli-ai/cloudcli@latest` dans le conteneur. HolyClaude inclut un runtime CloudCLI patché ; mettez à jour avec `docker compose pull && docker compose up -d`.
...
**Cause :** Un script de service CloudCLI personnalisé ou modifié n'a pas défini `WORKSPACES_ROOT=/workspace` avant de lancer CloudCLI.
...
**Correctif :** Déjà géré — `entrypoint.sh` restaure d'abord le fichier de session enregistré, ou crée un fichier par défaut sûr quand aucune session enregistrée n'existe.
</details>

Consultez [docs/troubleshooting.md](../troubleshooting.md) pour le guide complet incluant tous les problèmes SMB/CIFS et l'historique complet des bugs rencontrés et corrigés.
