...
The full image includes EAS CLI 23.2.0 and Vercel CLI 59.13.1. EAS CLI bundles `tar` 7.5.19, while Vercel's `@vercel/fun` package bundles `tar` 7.5.7. The Docker build replaces only those two installed copies with checksum-verified `tar` 7.5.22, updates their exact dependency metadata, and fails if the parent package versions or dependency specs drift.
