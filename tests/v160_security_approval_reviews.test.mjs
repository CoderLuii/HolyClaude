...
test('retains the historical Chromium inventory without carrying its reviews into Chromium 153', () => {
  ...
  for (const target of targets) {
    ...
    assert.deepEqual(reviews, [], target);
  }
});

test('retains the nine historical scanner findings without active Chromium 152 mappings', () => {
  ...
  assert.equal(expectedVendorSeverities.size, 9);
});

test('keeps the historical raw High evidence but removes the fixed CVE-2026-87628 exception', () => {
  for (const target of targets) {
...
test('retires the five Chromium Critical approvals and exact Debian authority records', () => {
  ...
  for (const target of targets) {
    ...
    assert.equal(review, undefined, `${target} Critical review`);
...
if (process.env.HOLYCLAUDE_PORTABILITY_CHILD !== '1') {
  test('the evaluator maps every new raw Chromium finding exactly once', () => {
    ...
    try {
      ...
      const effectiveSeverities = new Map([
        ['CVE-2026-87534', 'Medium'],
        ['CVE-2026-87544', 'Low'],
        ['CVE-2026-87471', 'Medium'],
        ['CVE-2026-87433', 'Medium'],
      ...
      const reviews = ['High', 'Medium', 'Low'].map((severity) => ({
        id: `fixture-chromium-${severity.toLowerCase()}`,
        vulnerabilities: findings
          .map(({ vulnerability }) => vulnerability)
          .filter((vulnerability) => effectiveSeverities.get(vulnerability) === severity),
        component: {
          names: ['chromium', 'chromium-common', 'chromium-sandbox'],
          versions: ['152.0.7977.82-1~deb12u1'],
          types: ['deb'],
          locationPatterns: ['^/var/lib/dpkg/status$'],
        },
        disposition: 'vendor_severity',
        effectiveSeverity: severity,
        owner: 'Debian Bookworm Chromium',
        authority: {
          name: 'Chrome Releases',
          url: 'https://chromereleases.googleblog.com/2026/09/stable-channel-update-for-desktop_0808145027.html',
        },
        rationale: 'Historical evaluator fixture for exact Chromium 152 vendor severity mapping.',
      ...
      const result = spawnSync(process.execPath, [evaluator,
        ...
        '--as-of', '2026-09-18'], { encoding: 'utf8' });
