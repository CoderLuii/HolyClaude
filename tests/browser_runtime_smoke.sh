...
TSGOLINT_HELPER="$SCRIPT_DIR/cloudcli_tsgolint_runtime_checks.sh"
if [ ! -f "$HELPER" ]; then
...
if [ ! -f "$TSGOLINT_HELPER" ]; then
  echo "missing helper: $TSGOLINT_HELPER" >&2
  exit 1
fi
...
docker_literal_cmd cp "$TSGOLINT_HELPER_HOST_PATH" "$CONTAINER:/tmp/cloudcli_tsgolint_runtime_checks.sh"
docker_literal_cmd exec "$CONTAINER" chmod 0755 /tmp/browser_runtime_container_checks.sh
docker_literal_cmd exec "$CONTAINER" chmod 0755 /tmp/cloudcli_tsgolint_runtime_checks.sh
docker_literal_cmd exec \
  -u claude \
  "$CONTAINER" \
  /tmp/cloudcli_tsgolint_runtime_checks.sh
docker_literal_cmd exec \
  -u claude \
