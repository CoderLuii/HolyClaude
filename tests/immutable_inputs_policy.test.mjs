...
function runFixture(mutate = (value) => value) {
  ...
  try {
    ...
    return spawnSync(
      process.execPath,
      [validator, '--file', input, '--as-of', asOf, '--release', 'v1.6.1'],
      { encoding: 'utf8' },
    );
  } finally {
...
test('rejects immutable input evidence for another release', () => {
  const result = runFixture((value) => value.replace('release: v1.6.1', 'release: v1.5.1'));
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /expected release v1\.6\.1/);
});
...
test('rejects immutable input evidence reviewed after the deterministic as-of date', () => {
  const result = runFixture((value) => value.replace(`reviewed-at: ${reviewedAt}`, 'reviewed-at: 2026-09-15'));
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /reviewed-at 2026-09-15 is after as-of 2026-09-14/);
});
...
test('rejects duplicate top-level keys instead of silently overriding them', () => {
  const result = runFixture((value) => `release: v1.6.1\n${value}`);
  assert.notEqual(result.status, 0);
